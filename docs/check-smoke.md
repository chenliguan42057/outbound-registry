...
| # | 输入行 | 期望结果 |
|---|--------|----------|
| P1 | `冻干精华液 20支装 95` | name="冻干精华液 20支装"，qty=95，candidates=["冻干精华液 20支装"]，ambiguous=false |
...
| P5 | `冻干精华液 5支装\t320\t` | 表格粘贴：取最后数字列 qty=320，name="冻干精华液 5支装" |
| P6 | `冻干精华液 单支装`（无数量） | qty=null（只核存在性） |
...
## 二、compare 5 类结果用例（系统库存：冻干精华液 20支装=95 / 面膜 5片装=139 / 洁面慕斯 150ml=85 / 舒缓精粹霜 1g=127）

| # | 粘贴内容 | 期望分类 |
|---|----------|----------|
| C1 | `冻干精华液 20支装 95` | ✅ ok（一致） |
| C2 | `面膜 5片装 120` | ⚠️ warn（系统 139 vs 截图 120，差 -19） |
| C3 | `新货品试用装 10` | ❌ nf（系统无此货品） |
| C4 | `冻干精华液 单支装`（无数量） | ✅ ok（存在，不比较数量） |
| C5 | `面膜 5` | 2 行 warn，各标「名称不唯一」 |
| C6 | `随便写一行没有数字的东西` | ⚪ unk（行格式异常） |
| C7 | `洁面慕斯 50ml 100000` | ⚪ unk（数量超范围） |
| C8 | `冻干精华液 20支装 95` 单独核对 | 其余 19 个货品 → 🔵 miss（截图未提及，全量列出） |
...
| E7 | `Engine.checkStock("冻干精华液 20支装 95")` | type="check"，table.rows[0].cls="ok" |
