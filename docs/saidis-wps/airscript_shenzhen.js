...
// ===== 产品列管理（2026-09-05）：目录添加/删除货品 → 金山台账加/删 [发放,库存] 两列 =====
// 触发：前端保存货品目录 diff 出增删 → GitHub Action 调 webhook
//   mode:"add_product_col"   {sheet_name, col}   col=归一化列名（如 精华液40支装；老映射如 20支盒）
//       在子表右侧（记录ID列左侧）插入两列，表头「发放{col}」「库存{col}」；列已存在则幂等返回。
//   mode:"del_product_col"   {sheet_name, col}
//       找到该产品的 [发放,库存] 两列并删除（含整列历史数据，前端已强提醒）；找不到则幂等 notFound。
// 注意：新增列的表头文字会参与 scanProductPairs 的归一化匹配，normalize("发放"+col) === col，
//       因此后续 append_order 把该产品写进新列，与其他老列行为完全一致。

function insertColumnAt(sh, idx) {
  var letter = colLetter(idx);
  var errs = [];
  var tryFns = [
    function () { sh.Columns(letter + ":" + letter).Insert(); },
    function () { sh.Columns(letter).Insert(); },
    function () { sh.Range(letter + "1").EntireColumn.Insert(); }
  ];
  for (var i = 0; i < tryFns.length; i++) {
    try { tryFns[i](); return true; } catch (e) { errs.push(String((e && e.message) || e)); }
  }
  throw new Error("插入列失败(" + letter + "): " + errs.join(" | "));
}

function deleteColumnAt(sh, idx) {
  var letter = colLetter(idx);
  var errs = [];
  var tryFns = [
    function () { sh.Columns(letter + ":" + letter).Delete(); },
    function () { sh.Columns(letter).Delete(); },
    function () { sh.Range(letter + "1").EntireColumn.Delete(); }
  ];
  for (var i = 0; i < tryFns.length; i++) {
    try { tryFns[i](); return true; } catch (e) { errs.push(String((e && e.message) || e)); }
  }
...
function addProductCol(a) {
  ...
  if (!sheetName) throw new Error("add_product_col 缺少 sheet_name");
  if (!col) throw new Error("add_product_col 缺少 col");
  var sh = Application.Sheets(sheetName);
  if (!sh) throw new Error("找不到子表: " + sheetName);
  ensureHeaders(sh, sheetName);
  // 幂等：该归一化列已存在（无论列名叫什么，只要 normalize 后 == col）
  if (scanProductPairs(sh)[col]) {
...
function delProductCol(a) {
  var sheetName = a.sheet_name;
  var col = String(a.col || "").trim();
  if (!sheetName) throw new Error("del_product_col 缺少 sheet_name");
  if (!col) throw new Error("del_product_col 缺少 col");
  var sh = Application.Sheets(sheetName);
  if (!sh) throw new Error("找不到子表: " + sheetName);
  var pair = scanProductPairs(sh)[col];
  if (!pair) {
    console.log("产品列不存在，视为已删: " + col);
    return { ok: true, notFound: true, sheet: sheetName, col: col };
  }
  var issueCol = pair[0], stockCol = pair[1];
  deleteColumnAt(sh, stockCol);    // 先删右侧库存列，避免列号错位
  deleteColumnAt(sh, issueCol);    // 再删发放列
  console.log("已删除产品列: " + col + " (第" + colLetter(issueCol) + "/" + colLetter(stockCol) + "列)");
  return { ok: true, sheet: sheetName, col: col, deletedCols: [issueCol, stockCol] };
}
...
function main() {
  ...
  try {
    ...
    if (a.mode === "add_product_col") {
      ...
      console.log("===== 产品列新增完成: " + JSON.stringify(apc));
      return apc;
    }

    // ---- 产品列删除（目录删货品 → 金山删对应两列，幂等）----
    if (a.mode === "del_product_col") {
      var dpc2 = delProductCol(a);
      console.log("===== 产品列删除完成: " + JSON.stringify({
        notFound: dpc2.notFound, deletedCols: dpc2.deletedCols }));
      return dpc2;
    }
