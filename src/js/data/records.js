...
(function () {
  ...
  function toCsv(list) {
    ...
    var head = ["序号", "时间", "申请人", "领取人", "部门", "用途/项目", "备注", "货物名称及数量", "库存", "照片数"];
    if (hasOut) head.splice(4, 0, "状态", "出货仓库单位");   // 状态/出货仓库单位列插在「领取人」「部门」之间（保持既有状态列 splice 语义）
    var rows = list.map(function (r, i) {
      ...
      var row = [list.length - i, r.time || "", r.applicant || "", r.picker || "", r.dept || "", r.purpose || "", r.note || "", items, stocks, (r.photos || []).length];
      if (hasOut) row.splice(4, 0, getStatus(r) === "pending" ? "未提单" : "已提单", r.entity || "");
      return row;
    });
