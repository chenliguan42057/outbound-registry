...
(function () {
  ...
  /** 转为出库记录 payload（确认出库时调用；不含 confirmed/shipped）。
      出货仓库单位：待取货表单不收集，按当前系统实体补全（Config.Sys.entity()：深圳系统=深圳细胞法人、
      赛迪斯系统=赛迪斯法人），与出库登记表单的空值回退规则一致（out.js DEFAULT_ENTITY）；备注原样透传。 */
  function toOutboundPayload(pk) {
    var entity = (window.App.Config && window.App.Config.Sys && window.App.Config.Sys.entity()) || "深圳细胞法人";
    return {
      time: Util.nowLocal(),
      picker: pk.picker,
      dept: pk.dept,
