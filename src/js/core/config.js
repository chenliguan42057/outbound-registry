...
(function () {
  ...
  /* 商品目录（精简名）来自「单一真相源」product-map.js（window.APP_PRODUCT_MAP.products），
     与后台 wps_sync.py 共用同一份，避免两端映射漂移导致"列对不上"。
     若脚本未加载（极端情况），用下方内联兜底，保证页面不崩。 */
  var PRODUCTS = (window.APP_PRODUCT_MAP && window.APP_PRODUCT_MAP.products)
  ...
  var INVENTORY = {
    "精华液 20支装": 97, "精华液 5支装": 320, "精华液 单支装": 222, "面膜 5片装": 139, "面膜 1片装": 218,
    "精华液 30支装": 42, "洁面慕斯 150ml": 85, "洁面慕斯 50ml": 77, "精粹水 120ml": 62, "精粹水 30ml": 52,
    "精粹乳 80ml": 88, "精粹乳 30ml": 66, "精粹乳 1ml": 124, "精粹霜 50g": 60, "精粹霜 15g": 82, "精粹霜 5g": 99, "精粹霜 1g": 127,
    "华大鹿茸凝时系列礼盒装": 4,
    "小鹿牛皮纸袋 大": 199,
    "小鹿牛皮纸袋 小": 147
  };
  ...
  /* 改名映射（旧名→新名）同样来自 product-map.js（nameMap），与后台共用。
     ...
     兜底内联同上。 */
  var NAME_MAP = (window.APP_PRODUCT_MAP && window.APP_PRODUCT_MAP.nameMap)
  ...
  var Config = {
    ...
    CATEGORY_MAP: {
      "精华液": ["精华液 20支装", "精华液 5支装", "精华液 单支装", "精华液 30支装"],
      "面膜": ["面膜 5片装", "面膜 1片装"],
    ...
    /* 自动同步（电脑端定时从云端拉取手机端提交的数据）。
       ...
       当前 data/records 30 + data/deleted 9 + data/memos 3 + data/pickups 0 ≈ 46 次请求/轮。
       Contents API 限额 5000 次/小时（按令牌计，多设备共享）：
  ...
  /**
   * 解析云端令牌：优先部署注入的 window.__GH_TOKEN__（值为「令牌占位符」时视为未注入），
